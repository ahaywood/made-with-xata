...
describe("ButtonLink", () => {
  ...
  it('displays a thick border', () => {
    ...
    expect(screen.getByTestId('border')).toHaveClass('p-[6px]', { exact: false });
  });

  it('displays a thin border', () => {
    render(<ButtonLink href={href} label={label} borderWidth="thin" />)
    expect(screen.getByTestId('border')).toHaveClass('p-[2px]', { exact: false });
  });

  it('shows the butterfly', () => {
    render(<ButtonLink href={href} label={label} isButterflyShowing={true} />)
    expect(screen.getByTestId('butterfly')).toBeInTheDocument();
  });

  it('does not show the butterfly', async () => {
